	...
	public void encodeMessage() throws SOAPEncodeException {
		...
		try {
			...
			if (mm7VaspReq.isTransactionIDExist()) //
				sb.append("<mm7:TransactionID xmlns:mm7=\"http://www.3gpp.org/ftp/Specs/archive/23_series/23.140/schema/REL-6-MM7-1-0\" env:mustUnderstand=\"1\">"
			...
			if (mm7VaspReq instanceof MM7SubmitReq) {
				 ...
				 * if(submitReq.isReplyChargingExist())
				 * sb.append("<ReplyCharging>"
				 * +submitReq.getReplyCharging()+"</ReplyCharging>");
				 * if(submitReq.isReplyDeadlineExist())
				 * sb.append("<Reply-Deadline>"+sdf.format(new Date(
				 * submitReq.getReplyDeadlineRelative
				 * ()))+"+08:00"+"</Reply-Deadline>");
				 * if(submitReq.isReplyDeadlineAbsoluteExist())
				 * sb.append("<Reply-Deadline>"+sdf.format(
				 * submitReq.getReplyDeadlineAbsolute
				 * ())+"+08:00"+"</Reply-Deadline>");
				 * if(submitReq.isReplyChargingSizeExist())
				 * sb.append("<ReplyChargingSize>"
				 * +submitReq.getReplyChargingSize()+"</ReplyChargingSize>");
				 */
				...
				if (submitReq.isContentExist()) {
					 ...
					 * sb.append("<Content href=\"cid:"
					 * +submitReq.getContent().getContentID()+"\"");
			 ...
			 * */
				...
				if (replaceReq.isContentExist()) {
					 ...
					 * sb.append("<Content href=\"cid:"
					 * +replaceReq.getContent().getContentID()+"\"");
